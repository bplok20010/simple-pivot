...
function pickName2() {
  var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var keys = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
  var vKey = arguments.length > 2 ? arguments[2] : undefined;
  var delimiter = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "_";
  var VALUE_MACRO = arguments.length > 4 ? arguments[4] : undefined;
  return keys.map(function (key) {
    return key === VALUE_MACRO ? vKey : data[key] + "";
  }).join(delimiter);
}
...
function simplePivot() {
  ...
  var VALUE_MACRO = options.valueMacro || "E_VALUE";

  if (pOpts.columns.indexOf(VALUE_MACRO) === -1) {
    pOpts.columns = [].concat(pOpts.columns, VALUE_MACRO);
  }

  var columnsWithoutMacro = pOpts.columns.filter(function (column) {
    return column !== VALUE_MACRO;
  });
  ...
  if (!pOpts.values.length) {
    ...
    (0, _warning["default"])(false, "options.values cannot be empty!"); //options.columns or
  }

  var undefColumns = (0, _difference["default"])([].concat(columnsWithoutMacro, pOpts.values), data.columns);

  if (undefColumns.length) {
    (0, _warning["default"])(false, "simplePivot unknow columns: " + undefColumns.join(","));
  }

  pOpts.rows = (0, _difference["default"])(data.columns, [].concat(columnsWithoutMacro, pOpts.values));
  var dataset = data.list.map(function (d) {
    return (0, _zipObject["default"])(data.columns, d);
  });
  var groupNames = []; //记录分组顺序

  var groupNameMaps = Object.create(null);
  ...
  var _loop = function _loop(i) {
    ...
    gData.forEach(function (item, i) {
      ...
      pOpts.values.forEach(function (vKey) {
        var columnName = pickName2(item, pOpts.columns, vKey, delimiter, VALUE_MACRO);
